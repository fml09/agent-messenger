...
`tablet` is the default Android sub-device profile. `android-main` is an
explicit experimental profile that sends `dtype=1` and must be limited to an
...
const client = await new KakaoTalkClient().login({
  ...
  userId,
  deviceUuid,
  deviceType: 'android-main',
})
```

The profile does not make message editing universally available; KakaoTalk
still decides whether the account and message satisfy its edit restrictions.
...
if (target) {
  // Search the current KakaoTalk reaction catalog instead of guessing a
  // numeric reaction type.
  const candidates = await client.searchReactions('엄지척')
  const reaction = candidates[0]
  if (!reaction) throw new Error('Reaction not found')

  await client.addReaction('9876543210', target.log_id, reaction.o)
  await client.removeReaction('9876543210', target.log_id, reaction.o)
}
```

`searchReactions(query)` calls KakaoTalk's authenticated reaction catalog
endpoint and returns catalog entries whose `o` field is the `reactionId` used
by `addReaction()` and `removeReaction()`. The current KakaoTalk client exposes
up to 114 catalog reactions; the available set can vary by account, app
version, and entitlement.

For OpenChat, pass the `linkId` in the options object:

```typescript
await client.addReaction('9876543210', target.log_id, reaction.o, { linkId: '77777' })
```

To inspect which users used each reaction on a message:

```typescript
const members = await client.getReactionMembers('9876543210', target.log_id)
// → [{ reactionId: '1200282_026', userIds: ['...'] }]
```

The reaction endpoints are private KakaoTalk APIs, not Kakao Developers APIs.
They may change without notice and should be exercised only with an account
you control.
